...
class Solution {
  public:
  vector<vector<int>> insert(vector<vector<int>>& intervals, vector<int>& newInterval) {
    if (intervals.size() == 0) vector<vector<int>> {newInterval};
    vector<vector<int>> loi;
    for (int i = 0; i < intervals.size(); i++) {
      if(newInterval[1] < intervals[i][0]) {
        loi.push_back(newInterval);
        copy(intervals.begin()+i, intervals.end(), back_inserter(loi));
        return loi;
      } else if(newInterval[0] > intervals[i][1]) loi.push_back(intervals[i]);
      else {
        newInterval = {
